...
Default project scans finalize scope before collection by combining `$HOME/Projects` with bounded metadata from VS Code, Cursor, Windsurf, JetBrains, and reciprocal Git linked worktrees. Automatic candidates must be verified local directories strictly below `$HOME`; symlinks, remote locations, external volumes, and excluded personal/application subtrees are rejected. Discovery executes no process or network call and persists no raw path, URI, workspace/product/worktree identifier, or metadata content. Any explicit `--project-root` is an exact override and disables all automatic metadata reads. GitHub release archives remain the distribution context; do not add App Store or Apple-signing work to this path.
