...
func TestContentEvidenceGoldenBaselineReport(t *testing.T) {
	...
	if document["schemaVersion"] != "ssc-init.scan.v6" {
...
func TestContentEvidenceCLIBaselineAndStatusEndToEnd(t *testing.T) {
	...
	if baselineReport.SchemaVersion != "ssc-init.scan.v6" || baselineReport.EvidenceCoverage == nil || baselineReport.Inventory == nil {
	...
	if status.SchemaVersion != "ssc-init.status.v6" || !status.Initialized || status.InventorySchemaVersion != "ssc-init.scan.v6" || status.LegacyInventory {
