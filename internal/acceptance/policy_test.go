...
func TestPolicyNeverEntersTheScanContract(t *testing.T) {
	...
	scanner := policyAcceptanceScanner{
		scan:      model.ScanResult{SchemaVersion: "ssc-init.scan.v7", ScanID: "scan-isolated", Status: model.ScanComplete, StartedAt: now, FinishedAt: now},
...
func policySnapshotAt(assetID, projectID, digest string, at time.Time) model.Snapshot {
	return model.Snapshot{
		Scan: model.ScanResult{SchemaVersion: "ssc-init.scan.v7", ScanID: "scan-1", Status: model.ScanComplete, FinishedAt: at},
		Inventory: model.Inventory{
