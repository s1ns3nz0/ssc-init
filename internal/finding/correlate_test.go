...
func TestCorrelateMatchesExactAssetAndHashAndKeepsBenignTwinClear(t *testing.T) {
	now := time.Date(2026, 8, 10, 0, 0, 0, 0, time.UTC)
	badHash := strings.Repeat("a", 64)
	inventory := model.Inventory{Assets: []model.Asset{
		{ID: "ide-extension:vscode:bad@1.0.0", Type: model.AssetIDEExtension, Name: "bad", Version: "1.0.0", SHA256: badHash},
		{ID: "ide-extension:vscode:benign@1.0.0", Type: model.AssetIDEExtension, Name: "benign", Version: "1.0.0", SHA256: strings.Repeat("b", 64)},
	}}
	active := activeTI([]bundle.TIRecord{{ID: "ti:bad", AssetID: inventory.Assets[0].ID, SHA256: badHash, Verdict: "known-malicious", Confidence: "high", CampaignIDs: []string{"campaign-1"}, AttackTechniques: []string{"T1059.007"}}})
...
func TestCorrelateRejectsHashMismatchWithdrawnAndRangeOnlyRecords(t *testing.T) {
...
func TestCorrelateMergesEvidenceDeterministicallyByStrongestVerdict(t *testing.T) {
	asset := model.Asset{ID: "tool:example", Type: model.AssetTool, Name: "example"}
	records := []bundle.TIRecord{
		{ID: "z-record", AssetID: asset.ID, Verdict: "needs-review", Confidence: "low"},
		{ID: "a-record", AssetID: asset.ID, Verdict: "suspicious", Confidence: "medium"},
	}
