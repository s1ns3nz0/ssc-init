...
// UnnamedAsset is the display placeholder for a digest-anchored asset whose
// readable inventory record is no longer present.
const UnnamedAsset = unnamedAsset
...
// LadderAssetTypes returns a copy of the ID-prefix to display-type catalog.
func LadderAssetTypes() map[string]string {
	result := make(map[string]string, len(assetTypeByIDPrefix))
	for prefix, assetType := range assetTypeByIDPrefix {
		result[prefix] = assetType
	}
	return result
}
...
// ParseAssetID exposes the one asset-ID parser shared by policy and report.
func ParseAssetID(id string) (assetType, host, name, version string) { return parseAssetID(id) }

// IsDigestSegment reports whether value is a bare anchored SHA-256 segment.
func IsDigestSegment(value string) bool { return digestSegment.MatchString(value) }
...
// Rung is one severity-ladder classification. Its ordinal is the documented
// display order and the lower ordinal wins when several facts describe one
// asset.
type Rung = rung
...
const (
	...
	RungUnverified = rungUnverified
	RungUpgraded   = rungUpgraded
	RungRemoved    = rungRemoved
)

// Label returns the closed public label for this rung.
func (value rung) Label() string { return rungLabels[value] }

// RungByLabel resolves one exact documented ladder label.
func RungByLabel(label string) (Rung, bool) {
	for value, candidate := range rungLabels {
		if candidate == label {
			return value, true
		}
	}
...
// Row is one classified asset row. The ordering key remains private so it can
// never leak through rendering or JSON.
type Row = rungRow

// AssetID returns the asset this row classifies without exposing the private
// total-order key as a serializable field.
func (row rungRow) AssetID() string { return row.key }

// Ladder classifies a delta into at most one highest-severity row per asset.
func Ladder(inventory model.Inventory, delta model.Delta) []Row { return classify(inventory, delta) }
