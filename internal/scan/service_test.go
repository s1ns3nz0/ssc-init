...
func TestBaselinePersistsPartialScanAndDiffsPreviousInventory(t *testing.T) {
	...
	if result.Status != "partial" || result.SchemaVersion != "ssc-init.scan.v5" {
...
func TestBaselineBuildsAndPersistsV3ScopeAndDropsLocalTargets(t *testing.T) {
	...
	if result.SchemaVersion != "ssc-init.scan.v5" || !reflect.DeepEqual(result.Scope, wantScope) {
		t.Fatalf("result=%+v", result)
	}
...
func TestBaselineCollectsEvidenceAfterGraphNormalization(t *testing.T) {
	...
	if scanResult.SchemaVersion != "ssc-init.scan.v5" || len(inventoryResult.Evidence) != 1 || scanResult.EvidenceCoverage.Status != model.CoverageComplete {
	...
	if snapshots.saved[0].SchemaVersion != "ssc-init.scan.v5" || !reflect.DeepEqual(snapshots.saved[0].EvidenceCoverage, scanResult.EvidenceCoverage) {
