...
func TestCatalogDeclaresOfficialMCPBoundary(t *testing.T) {
	...
	want := []expectedDeclaration{
		...
		{id: "mcp.codex.project", relativePath: ".codex/config.toml", host: "codex", format: "toml", consumers: []string{"codex"}, scope: model.ScopeProject, method: model.TargetFile, supported: true},
		{id: "mcp.codex.user", relativePath: ".codex/config.toml", host: "codex", format: "toml", consumers: []string{"codex"}, scope: model.ScopeUser, method: model.TargetFile, supported: true},
